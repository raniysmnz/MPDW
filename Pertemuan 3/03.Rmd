...
## Input data
```{r}
df <- rio::import("https://raw.githubusercontent.com/raniysmnz/MPDW/main/Pertemuan%203/wine_fixed.csv")
data <- df[14:33,]
str(data)
```
## Pembagian data
...
$$
\hat{Y_t}=235.6779+0.4948X_t+0.08289Y_{t-1}
$$
...
$$
\hat{Y_t}=-273.287+0.0274X_t+0.03995X_{t-1}+0.7793X_{t-2}
$$
...
$$
\hat{Y_t}=831.2558-0.052525X_t+...+0.03601X_{t-6}
$$
...
$$
\hat{Y}=235.0727+0.1808X_t+0.5065X_{t-1}+0,.53869Y_{t-1}
$$
...
Dari tabel di atas, dapat terlihat bahwa nilai AIC terendah didapat ketika $p=12$ dan $q=2$, yaitu sebesar `104,5672`. Artinya, model autoregressive optimum didapat ketika $p=12$ dan $q=2$.
...
```{r}
par(mfrow=c(1,1))
plot(test$x, test$yt, type="b", col="black", main=(""))
...
Berdasarkan plot dan data MAPE tiap pemodelan tersebut, terlihat bahwa plot yang paling mendekati data aktualnya adalah Model DLM 1, sehingga dapat disimpulkan model terbaik dalam hal ini adalah model DLM 1
